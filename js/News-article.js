...
function renderNews() {
    ...
    if (iTotalNews) {
        ...
        if (entry1) {
            ...
            oNewsContainer.append(oNewsArticlePager.template.replace(/@title/g, sTitle).replace("@date", oDate).replace("@content", sContent).replace(/@newsimagesrc/g, src).replace("@tooltip", getFirstNWordsWithEllipses(sRawTitle, 4)).replace("@fbunewslink", "https://maqsoftware.com/news-article?title=" + sRawTitle).replace("@newslink", "https://maqsoftware.com/news-article?title=" + sRawTitle).replace("@twitternewslink", encodeURI("https://maqsoftware.com/news-article?title=" + sRawTitle)).replace("@linkedinnewslink", encodeURI("https://maqsoftware.com/news-article?title=" + sRawTitle))
